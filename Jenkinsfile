pipeline {
    ...
    stages {
        ...
        stage('Helm Install') {
            steps {
                withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'KubCluster', contextName: '', credentialsId: 'kub_secret', namespace: 'dev', serverUrl: 'https://192.168.1.24:6443']]) {
                    sh 'helm install fastapiapp-dev ./fastapiapp-0.1.0.tgz --namespace dev -f fastapiapp/values-dev.yaml'
}
            }
        }
    }
