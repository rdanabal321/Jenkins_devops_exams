pipeline {
    ...
    stages {
        ...
        stage('Helm Install') {
            steps {
                withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: 'KubCluster', contextName: '', credentialsId: 'kub_secret', namespace: 'qa', serverUrl: 'https://192.168.1.24:6443']]) {
                    sh 'helm install fastapiapp-qa ./fastapiapp-0.1.0.tgz --namespace qa -f fastapiapp/values-qa.yaml'
}
            }
        }
    }
